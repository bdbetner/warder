...
Landlock is the preferred mechanism for preventing writes to protected paths. Path checks canonicalize where possible and reject traversal or unsafe overlaps in config, policy, snapshot, and enforcement planning paths. Warder also re-validates Landlock rule paths immediately before `landlock_restrict_self` so a last-moment symlink or overlap replacement fails closed instead of silently using stale planning. Missing paths and symlinks are handled deliberately so receipts can describe what was actually enforced or degraded.
...
Warder refuses launches when the SQLite database path or strict-mode receipt key path is below a configured zone path or agent writable root. On Unix, existing parent directories for those state files must also be private (`0700` or stricter), and writable shared ancestors are rejected unless they use the sticky bit convention of `/tmp`-style directories. Agent writable roots are the paths Landlock allows the child to write, and zone paths are the session's sensitive policy surface, so placing Warder state there would let a successful supervised session tamper with or entangle its own local accountability records. These checks are placement and permission guards, not tamper-proof forensics; unrelated same-user processes remain outside Warder's control.
