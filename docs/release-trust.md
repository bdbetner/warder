...
Warder public beta releases use a GitHub Release artifact folder as the distribution
...
Package-manager signatures are not included in current public beta releases.

Treat every release as a review build until the installed-artifact demo in [Release Readiness](release-readiness.md) passes on a clean Linux machine.
...
attestations enabled. Some builds may only have checksum, manifest, and
package-smoke verification.

## Full Verification Flow

Use this flow before installing a downloaded public beta release:
...
the release look more mature than it is. For the current public beta channel, keep the
