...
- **Pre-exec supervised setup**: Warder-launched commands are assigned to a session cgroup, covered by seccomp deny-list hardening, given best-effort cgroup resource limits, and locked down with Landlock before the target command is executed, where the host supports those features.
