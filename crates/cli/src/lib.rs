...
paths = [{zone_paths}]\n\
write_policy = \"deny\"\n\
# Experimental read blocking is off by default. To opt in, set read-deny = true\n\
# and configure disjoint enforcement.readable_roots above.\n\
read-deny = false\n\
snapshot = \"disabled\"\n\n\
[[agents]]\n\
id = {agent_id}\n\
...
pub fn render_host_readiness(report: &HostReadinessReport) -> String {
    ...
    lines.push(format!(
        "supervision scope: {GLOBAL_SUPERVISION_LIMITATION}"
    ));
    lines.join("\n")
}
...
pub fn render_host_doctor(report: &HostDoctorReport) -> String {
    ...
    append_ebpf_tracepoint_status_table(&mut lines);
    lines.join("\n")
}
...
fn append_ebpf_tracepoint_status_table(lines: &mut Vec<String>) {
    lines.push("eBPF tracepoint status:".to_string());
    let file_available = warder_journal::live_ebpf_file_attach_available();
    let network_available = warder_journal::live_ebpf_network_attach_available();
    for (program, tracepoint) in warder_journal::default_ebpf_file_tracepoints() {
        let status = if file_available {
            "attach-planned"
        } else {
            "degraded"
        };
        lines.push(format!("- file {tracepoint}: {status} ({program})"));
    }
    for (program, tracepoint) in warder_journal::default_ebpf_network_tracepoints() {
        let status = if network_available {
            "attach-planned"
        } else {
            "degraded"
        };
        lines.push(format!("- network {tracepoint}: {status} ({program})"));
    }
}
...
pub fn render_host_doctor_from_probe_with_config(
    ...
    if let Some(config_path) = config_path {
        let config = load_config(&config_path)?;
        append_config_policy_diagnostics(&mut report, &config);
        append_agent_command_diagnostics(&mut report, &config);
    }
    Ok(render_host_doctor(&report))
}

fn append_agent_command_diagnostics(report: &mut HostDoctorReport, config: &WarderConfig) {
...
fn append_config_policy_diagnostics(report: &mut HostDoctorReport, config: &WarderConfig) {
    if config
        .zones
        .iter()
    ...
    {
        report.diagnostics.push(HostDiagnostic {
            label: "experimental read denial".to_string(),
            status: HostDiagnosticStatus::Warning,
            message: "read denial is experimental and may break agents unless enforcement.readable_roots covers every required runtime, shell, model, and dependency path".to_string(),
        });
    }
}
...
fn planned_launch_visibility_limits(config: &WarderConfig) -> Vec<String> {
    ...
    limits.push(GLOBAL_SUPERVISION_LIMITATION.to_string());
    if config
        .zones
        .iter()
        .any(|zone| zone.effective_read_policy() == ReadPolicy::Deny)
    {
        limits.push(
            "read denial is experimental and may break agents that need dependency, shell, model, or runtime files not included in enforcement.readable_roots"
                .to_string(),
        );
    }
        ...
        config
            ...
            .flat_map(|zone| {
                zone.paths.iter().map(|path| LandlockRule {
                    path: path.clone(),
                    access: if zone.effective_read_policy() == ReadPolicy::Deny {
                        LandlockAccess::NoAccess
                    } else {
                        match zone.write_policy {
